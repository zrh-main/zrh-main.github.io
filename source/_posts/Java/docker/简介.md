...
Docker  就是一个 360软件管家功能
     搜索软件 http://www.360.com/search     docker search  mysql
    下载地址:  http://www.360.com        Registry（注册中心）                          
1.下载软件 易信_4.1280.1941.0.exe      镜像        拉取
2.安装软件 安装好后的软件            容器           
3.使用                               使用容器
...
5.1.2创建MySQL容器
docker run -di --name kaola_mysql -p 33306:3306 -e MYSQL_ROOT_PASSWORD=123456 mysql
-p 代表端口映射，格式为  宿主机映射端口:容器运行端口
-e 代表添加环境变量  MYSQL_ROOT_PASSWORD是root用户的登陆密码
5.1.3进入MySQL容器,登陆MySQL 
进入mysql容器
docker exec -it kaola_mysql /bin/bash
登陆mysql
mysql -u root -p
5.1.4远程登陆MySQL
...
（2）在本地客户端执行建库脚本
执行“资源/建库语句/kaoladb.sql”

5.1.5查看容器IP地址
我们可以通过以下命令查看容器运行的各种数据
docker inspect kaola_mysql
运行效果如下：
...
docker pull nginx
5.3.2创建Nginx容器
docker run -di --name=kaola_nginx -p 80:80  nginx/bin/bash
5.3.3测试Nginx
...
（1）从容器拷贝配置文件到宿主机
docker cp kaola_nginx:/etc/nginx/nginx.conf nginx.conf 
...
server {
	listen 80;
	server_name passport.kaola.com;
	location / {
...
（3）将修改后的配置文件拷贝到容器
docker cp nginx.conf  kaola_nginx:/etc/nginx/nginx.conf
（4）重新启动容器
docker restart kaola_nginx
（5）设置域名指向
192.168.247.135 passport.kaola.com 
浏览器测试：http://passport.kaola.com/cas/login

5.4 Redis部署
5.4.1拉取Redis镜像
docker pull redis
5.4.2创建Redis容器
 docker run -di --name=kaola_redis -p 6379:6379 redis
...
6.1容器保存为镜像
我们可以通过以下命令将容器保存为镜像
docker commit kaola_nginx mynginx
kaola_nginx是容器名称
mynginx是新的镜像名称
